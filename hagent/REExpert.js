var REExpert = function(_me, _M, _A, _s1, _s2, _attack0, _attack1)
{
	...
	this.acts = [];
	this.acts[0] = [this.s1 / this.A[1], this.s2/ this.A[1]];
	...
	this.makeAssessment = function(index)
	{
		...
		while(true)
		{
			...
			this.updateHist(this.hist, acciones);
			estado = this.state(this.hist);
		}
	...
	this.state = function(hist)
	{
		...
		for(var i = this.omega - 1; i >= 0; i--)
		{
			...
			newState += this.tier * this.encodeJointAction(hist[i]);
